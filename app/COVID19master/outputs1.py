...
class output_var:
    ...
    def write_current_results(self):
        ...
        df3 = pd.DataFrame({'Date': self.date_range,
                           ...
                           'number of universal testing needed': self.T_u_plot})
        ...
        df5 = pd.DataFrame({'Date': self.date_range,
                           ...
                           'number of hospitalized - age group 1': self.tot_hosp_AgeGroup1_plot,
                           'number of hospitalized - age group 2': self.tot_hosp_AgeGroup2_plot,
                           'number of hospitalized - age group 3': self.tot_hosp_AgeGroup3_plot,
                           'number of hospitalized - age group 4': self.tot_hosp_AgeGroup4_plot,
                           'number of hospitalized - age group 5': self.tot_hosp_AgeGroup5_plot,
                           'number of hospitalized - age group 6': self.tot_hosp_AgeGroup6_plot,
                           'number of hospitalized - age group 7': self.tot_hosp_AgeGroup7_plot,
                           'number of hospitalized - age group 8': self.tot_hosp_AgeGroup8_plot,
                           'number of deaths - age group 1': self.tot_dead_AgeGroup1_plot,
                           'number of deaths - age group 2': self.tot_dead_AgeGroup2_plot,
                           'number of deaths - age group 3': self.tot_dead_AgeGroup3_plot,
                           'number of deaths - age group 4': self.tot_dead_AgeGroup4_plot,
                           'number of deaths - age group 5': self.tot_dead_AgeGroup5_plot,
                           'number of deaths - age group 6': self.tot_dead_AgeGroup6_plot,
                           'number of deaths - age group 7': self.tot_dead_AgeGroup7_plot,
                           'number of deaths - age group 8': self.tot_dead_AgeGroup8_plot})
    ...
    def write_current_results_mod(self):
        ...
        # write to a new file 
        # writer = pd.ExcelWriter('results/{0}_result.xlsx'.format(self.State), engine = 'xlsxwriter')
    ...
    def write_output(self, pre_results):
        ...
        # write to a new file 
        writer = pd.ExcelWriter('results/{0}_final_result.xlsx'.format(self.State), engine = 'xlsxwriter')
    ...
    # def write_scenario_needed_results(self, pre_results, p, c, a_c,  SAR, R0, a_u):
    def write_scenario_needed_results(self, p, c, a_c,  SAR, R0, a_u, a_sd):
        df = pd.DataFrame({'Date': self.date_range,
                           ...
                           'number of new diagnosis through symptom-based testing': self.num_base,
                           'number of new diagnosis through universal testing':self.num_uni,
                           'total number of new diagnosis': self.num_inf_plot,
                           'number of contact tracing test needed': self.T_c_plot,
                           'number of universal testing needed': self.T_u_plot,
                           'Percent reduction in contacts through social distancing': self.policy_plot[:, 0],
                           'Testing capacity – maximum tests per day through contact tracing': self.policy_plot[:, 1],
                           ...
                           'number of quarantined': self.num_quarantined_plot,
                           'number of travel-related infection': self.travel_num_inf_plot})
        ...
        df2 = pd.DataFrame({'p': p,
                            ...
                            'Cumulative cost of testing': df['total cost of testing'].sum(),
                            'Cumulative number of contact tracing test needed': df['number of contact tracing test needed'].sum(),
                            'Cumulative number of universal testing needed': df['number of universal testing needed'].sum(),
                            'Cumulative diagnosis through contact tracing': df['number of new diagnosis through contact tracing'].sum(),
                            'Cumulative diagnosis through symptom-based testing': df['number of new diagnosis through symptom-based testing'].sum(),
                            'Cumulative diagnosis through universal testing': df['number of new diagnosis through universal testing'].sum(),
                            'Cumulative diagnosis': df['simulated cumulative diagnosis'][-1:].to_numpy()[0],
                            'Cumulative hospitalizations': df['simulated cumulative hospitalized'][-1:].to_numpy()[0],
                            'Cumulative deaths': df['simulated cumulative deaths'][-1:].to_numpy()[0], 
                            'Cumulative new infection': df['cumulative new infection'][-1:].to_numpy()[0],
                            'Peak contact tracing': df['number of contact tracing test needed'].max(),
                            'Peak quarantined': df['number of quarantined'].max()}, index=[0])         
    ...
    def write_scenario_needed_results_colab(self, pre_results):
        ...
        data = [df1['Date'],
                ...
                df5['number of new infection'],
                df5['cumulative new infection']]

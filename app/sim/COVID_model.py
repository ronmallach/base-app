...
class CovidModel():
    # def __init__(self, c_diag, c_dead, c_hosp, op_ob, pop_dist_dryrun):
    def __init__(self):
        ...
        self.time_unit = gv.time_unit      # user defined time unit 

        if self.time_unit == 'day':
            self.T = gv.T_max              # maximum simulation period for future
        elif self.time_unit == 'week':
            self.T = gv.T_max * 7          # maximum simulation period for future
        elif self.time_unit == 'year':
            self.T = gv.T_max * 365        # maximum simulation period for future
        ...
        self.VSL = gv.VSL
        self.md_salary = gv.md_salary
        self.K_val = gv.K_val
        self.A_val = gv.A_val
        self.h_val = gv.h_val
        self.Final_VSL = np.zeros(self.T_total + 1) 
        self.Final_SAL = np.zeros(self.T_total + 1)
        self.policy = np.zeros([self.T_total + 1,  3])        # social distancing, contact tracing, universal testing at n time step 
    ...
    def output_result(self):
        ...
        # self.op_ob.a_sd_plot[self.d - 1] = a_sd                            # a_sd at timestep t
        #### if plot for one day
    ...
    def calc_imm_reward(self, action_t):
        ...
        # print('self.Final_SAL[self.t]', self.Final_SAL[self.t])
        # input()
        # calculte total 'value of statistical life loss' due to deaths
        self.Final_VSL[self.t]  = np.sum(np.dot(self.num_dead[self.t] , self.VSL))
        # print('self.Final_VSL[self.t]', self.Final_VSL[self.t])
        # input()

        self.imm_reward[self.t] = -1 * (self.Final_VSL[self.t]  + self.Final_SAL[self.t] )
     
    # Function to calculate unemployment change
    def calc_unemployment(self):
        a_sd_max = self.a_sd_range[1]
        a_sd_n = self.policy[self.t][0]
        a_sd_p = self.policy[self.t - 1][0]
        prop = a_sd_n / a_sd_max - 1/2
        ...
        if y_p <= self.A_val:
            ...
            div = (self.K_val - self.rate_unemploy[self.t -1])/(self.rate_unemploy[self.t -1] - self.A_val)
            dz_dx = self.h_val * (self.K_val - self.A_val) * div/ (1 + div)**2
            if a_sd_n >= a_sd_p:
                dy_dt = dz_dx * (a_sd_n/a_sd_max)
            else:
                dy_dt = - dz_dx * (1 - a_sd_n/a_sd_max)

        self.rate_unemploy[self.t] = y_p + dy_dt
            ...
            for age in range (self.tot_age): # for each age group i.e., from 0-100
                ...
                for i1 in range(self.symp_hospitalization.shape[0]):
                    ...
                    if((age >= self.symp_hospitalization[i1, 0])&(age <= self.symp_hospitalization[i1, 1])):
                        ...
                        self.rate_array[12] = self.symp_hospitalization[i1,2]/self.qih_days
                        
                        self.rate_array[13]= (1 - self.symp_hospitalization[i1,2])/self.qir_days
                ...
                for i2 in range(self.percent_dead_recover_days.shape[0]):
                    if((age >= self.percent_dead_recover_days[i2,0])&(age <= self.percent_dead_recover_days[i2,1])):
                        self.rate_array[14] = (1 - (self.percent_dead_recover_days[i2,risk + 2]/100))/(self.percent_dead_recover_days[i2, 5])

                        self.rate_array[15] = (self.percent_dead_recover_days[i2,risk + 2]/100)/(self.percent_dead_recover_days[i2, 4])
                ...
                self.num_diag[self.t][risk][age] = (pop_dis_b[0,3] * self.rate_array[7])+(pop_dis_b[0,2]* self.rate_array[5])
  
                self.num_hosp[self.t][risk][age] = (pop_dis_b[0,6] * self.rate_array[12])
     
                self.num_dead[self.t][risk][age] = (pop_dis_b[0,7] * self.rate_array[15])
...
def setup_COVID_sim():
    ...
    beta_pre_sd = 1.12          # insert beta before social distancing
    beta_post_sd = 0.17         # insert beta after social distancing
    inv_dt = 10                 # insert time steps within each day
    T_max_ = 35                # insert maximum simulation time period since the dry run was done
    ...
    gv.setup_global_variables(state, beta_pre_sd, beta_post_sd, inv_dt,  T_max_, lead_time_, time_unit_, beta_user_defined_)

def run_COVID_sim():   
    
    sample_model = CovidModel()
    done = sample_model.t >= sample_model.T_total
    ...
    while done:
        ...
        print("##### step begin #####")
        ##### here beta can be user-defined beta 
        sample_model.t += 1 
        sample_model.simulation_base(action_t = sample_model.policy[sample_model.t], beta = gv.beta_before_sd)
        sample_model.calc_imm_reward(action_t = sample_model.policy[sample_model.t])
        
        if sample_model.t % sample_model.inv_dt == 0: 
            sample_model.d += 1 
            sample_model.output_result() 
            # print(sample_model.op_ob.SAL_plot[sample_model.d])
            # input()
          
        print("##### step end ##### \n")
        done = sample_model.t < sample_model.T_total
    print(sample_model.op_ob.SAL_plot)
    print(sample_model.op_ob.unemployment)
    sample_model.op_ob.plot_output()
